...
final class Describer
{
	...
	private function describeString(string $s)
	{
		...
		if ($res === $s) {
			...
			return new Model(['string' => $res, 'length' => strlen(utf8_decode($s))]);
		} else {
			return new Model(['string' => $res, 'length' => strlen($s), 'bin' => true]);
		}
	}
		...
		if (!$shot) {
			...
			if (isset($this->resourceExposers[$type])) {
				foreach (($this->resourceExposers[$type])($resource) as $k => $v) {
					$shot->items[] = [htmlspecialchars($k), $this->describeVar($v, $depth + 1)];
				}
			}
		}
